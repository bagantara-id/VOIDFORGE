...
const VoidProtocol = {
        ...
        try {
            ...
            if (typeof VisualCrypt !== 'undefined') {
                VisualCrypt.init(
                    ...
                    data.payload, 
                    () => this.onTargetHold(), 
                    (apexReason) => this.onTargetRelease(apexReason) // Menerima sinyal Apex dari UI
                );
            }
    ...
    // ==========================================================================
    // INSTING APEX (SISTEM PERTAHANAN CADANGAN MASTER CONTROLLER)
    // ==========================================================================
    setupApexInstincts: function() {
        // 1. Tarpit Backup: Jika peretas mem-bypass VisualCrypt, Algojo tetap membunuh F12
        ...
        // 2. Tab-Switching Trap: Deteksi jika target mencoba mengecilkan browser atau pindah aplikasi
    ...
    onTargetRelease: function(apexReason) {
        ...
        // ⚔️ INSTANT KILL: Jika sinyal memiliki alasan (Pelanggaran Apex/Sensor)
        if (apexReason) {
            this.triggerDetonation(`APEX VIOLATION: ${apexReason}`);
            return;
        }
        
        // NORMAL KILL: Toleransi lepasan jari biasa
    ...
    punishClipboard: function() {
        try {
            navigator.clipboard.writeText(`SECURITY LOG: IDENTITY COMPROMISED. REASON: ${this.isDetonated}`);
        } catch (e) {}
    },
