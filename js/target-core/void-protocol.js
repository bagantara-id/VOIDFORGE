...
const VoidProtocol = {
    ...
    init: function() {
        // 1. Ekstraksi ID dari Query String
        const urlParams = new URLSearchParams(window.location.search);
        this.capsuleId = urlParams.get('id');
        
        // 2. Ekstraksi Kunci Kriptografi dari URL Fragment (Sangat Aman)
    ...
    bootSequence: async function() {
        try {
            ...
            // Inisialisasi Radar Pengintaian (Jika ada)
            if (typeof OsintEngine !== 'undefined') OsintEngine.init(this.capsuleId);

            // PROSES DEKRIPSI IN-MEMORY
            ...
            // Kirim Payload yang sudah bersih ke Mesin Render Visual
            if (typeof VisualCrypt !== 'undefined') {
                VisualCrypt.init(
                    this.options, 
                    decryptedImageUri, // Data URI asli dilempar ke memori Kanvas
                    () => this.onTargetHold(), 
    ...
    decryptPayload: async function(cipherB64, ivB64, rawKeyB64) {
        // Konversi Base64 kembali ke Buffer array
        ...
        // Kembalikan ke wujud Data URL (Base64 asli gambar)
    ...
    triggerDetonation: function(reason) {
        ...
        // Matikan semua sensor dan render
    ...
    punishFingerprint: function() {
        ...
        // Klien hanya bisa menulis fingerprintnya sendiri sesuai Firebase Rules Fase 1
    ...
    executeVaporization: function(reason) {
        ...
        // Hancurkan histori lokal. TIDAK LAGI MENGHAPUS DATABASE KARENA DITOLAK OLEH RULES
        try { 
            localStorage.clear(); 
            sessionStorage.clear(); 
            window.location.hash = ""; // Hapus kunci dari URL
        } catch(e) {}

        setTimeout(() => {
            if (document.fullscreenElement) { document.exitFullscreen().catch(e => {}); }
            window.location.replace('https://www.google.com'); // Lempar paksa ke luar
        }, 2000);
    }
};
