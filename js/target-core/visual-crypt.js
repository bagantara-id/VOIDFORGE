 ...
 * Status: MILITARY-GRADE PATCHED (HOTFIX: Global Touch Tracking)
 */
        ...
        const updatePos = (e) => {
            if (this.isHolding) {
                ...
                if (this.options.corrosive) {
                    ...
                    if (this.touchTrail.length > 300) this.touchTrail.shift(); 
                }
            }
        };
        
        // PERBAIKAN MUTLAK: Mengubah sensor dari kanvas menjadi pelacakan global di document
        document.addEventListener('touchmove', updatePos, { signal: this.eventAborter.signal, passive: false });
        document.addEventListener('mousemove', updatePos, { signal: this.eventAborter.signal });
    },
